...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # ── RSL file section ─────────────────────────────────────────────────
        lf2 = ttk.LabelFrame(self, text="Step 2 (optional) – Select results file(s) (*.rsl.csv)")
        lf2.grid(row=1, column=0, columnspan=2, sticky="ew", **pad)

        self.rsl_listbox = tk.Listbox(lf2, width=70, height=4, selectmode=tk.EXTENDED)
        self.rsl_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=4, pady=4)

        sb_rsl = ttk.Scrollbar(lf2, orient=tk.VERTICAL, command=self.rsl_listbox.yview)
        sb_rsl.pack(side=tk.LEFT, fill=tk.Y)
        self.rsl_listbox.configure(yscrollcommand=sb_rsl.set)

        btn_frame2 = ttk.Frame(lf2)
        btn_frame2.pack(side=tk.LEFT, padx=4)
        ttk.Button(btn_frame2, text="Add files…",      command=self._add_rsl_files).pack(fill=tk.X, pady=2)
        ttk.Button(btn_frame2, text="Remove selected", command=self._remove_rsl_files).pack(fill=tk.X, pady=2)
        ttk.Button(btn_frame2, text="Clear all",       command=self._clear_rsl_files).pack(fill=tk.X, pady=2)
    ...
    def _add_rsl_files(self):
        ...
        for p in paths:
            if p not in self.rsl_files:
                self.rsl_files.append(p)
                self.rsl_listbox.insert(tk.END, os.path.basename(p))

    def _remove_rsl_files(self):
        selected = list(self.rsl_listbox.curselection())
        for i in reversed(selected):
            self.rsl_listbox.delete(i)
            self.rsl_files.pop(i)

    def _clear_rsl_files(self):
        self.rsl_listbox.delete(0, tk.END)
        self.rsl_files.clear()
    ...
    def _plot(self):
        ...
        if self.rsl_files:
            # Parse all RSL files and merge, deduplicating on datetime so that
            # overlapping sessions (two RSL files covering some of the same runs)
            # don't produce duplicate entries.
            all_rsl_records = []
            seen_dts = set()
            errors = []
            for rsl_path in self.rsl_files:
                try:
                    records = parse_rsl_csv(rsl_path)
                    for r in records:
                        dt_key = r["datetime"]
                        if dt_key is not None:
                            if dt_key in seen_dts:
                                continue
                            seen_dts.add(dt_key)
                        all_rsl_records.append(r)
                except Exception as e:
                    errors.append(f"{os.path.basename(rsl_path)}: {e}")
            if errors:
                messagebox.showerror("RSL parse error", "\n".join(errors))

            if all_rsl_records:
                rsl_with_dt = [r for r in all_rsl_records if r["datetime"] is not None]
                used_rsl = set()
        ...
        for item in to_plot:
            ...
            tag = "✓" if (self.rsl_files and item["datetime"] and
